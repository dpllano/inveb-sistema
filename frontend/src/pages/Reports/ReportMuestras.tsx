/**
 * ReportMuestras Component
 * Listado y estado de muestras
  ...
  ${({ $tipo }) => {
    switch ($tipo?.toLowerCase()) {
      case 'fisica':
        return `background: ${theme.colors.primary}20; color: ${theme.colors.primary};`;
      case 'digital':
        return `background: #7C3AED20; color: #7C3AED;`;
      case 'prototipo':
        return `background: ${theme.colors.warning}20; color: ${theme.colors.warning};`;
      default:
        return `background: ${theme.colors.textSecondary}20; color: ${theme.colors.textSecondary};`;
...
const tipoColors: Record<string, string> = {
  'fisica': theme.colors.primary,
  'digital': '#7C3AED',
  'prototipo': theme.colors.warning,
};
...
export default function ReportMuestras({ onNavigate }: ReportMuestrasProps) {
  ...
  const pieData = useMemo(() => {
    ...
    return {
      ...
      datasets: [{
        ...
        backgroundColor: apiData.por_tipo.map(t => tipoColors[t.tipo.toLowerCase()] || '#6B7280'),
      }],
    };
  }, [apiData]);
        ...
        <>
          <SummaryGrid>
            ...
            <SummaryCard $color={theme.colors.warning}>
              <SummaryValue>{estadoStats['pendiente'] || 0}</SummaryValue>
              <SummaryLabel>Pendientes</SummaryLabel>
            </SummaryCard>
            <SummaryCard $color={theme.colors.primary}>
              <SummaryValue>{estadoStats['en_proceso'] || 0}</SummaryValue>
              <SummaryLabel>En Proceso</SummaryLabel>
            </SummaryCard>
            <SummaryCard $color={theme.colors.success}>
              <SummaryValue>{estadoStats['completada'] || 0}</SummaryValue>
              <SummaryLabel>Completadas</SummaryLabel>
            </SummaryCard>
            <SummaryCard $color={theme.colors.danger}>
              <SummaryValue>{estadoStats['rechazada'] || 0}</SummaryValue>
          ...
          <ChartsGrid>
            {pieData && (
              <ChartCard>
                <ChartTitle>Por Tipo de Muestra</ChartTitle>
                <Pie data={pieData} options={pieOptions} />
              </ChartCard>
            )}
            {barChartData && (
              <ChartCard>
                <ChartTitle>Por Estado</ChartTitle>
          ...
          <ChartCardFull>
            <ChartTitle>Listado de Muestras ({apiData.total} registros)</ChartTitle>
            <Table>
              <thead>
                <tr>
                  <Th>ID</Th>
